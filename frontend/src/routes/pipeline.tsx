  ...
  return (
    <div className="space-y-8">
      ...
      <div className="overflow-x-auto">
        <div className="flex min-w-max items-center gap-2 pb-4">
            ...
            return (
              <div key={step.id} className="flex items-center gap-2">
                <button
                  ...
                  className={cn(
                    "glass w-52 shrink-0 rounded-xl p-4 text-left transition-all hover:-translate-y-0.5",
                    isActive && "ring-2 ring-primary shadow-glow"
                  )}
                >
                  <div className="flex items-center justify-between">
                    <Icon className="h-5 w-5 text-primary" />
                    <span className={cn("rounded-full border px-2 py-0.5 text-[10px]", statusStyle[step.status])}>{step.status}</span>
                  </div>
                  <h3 className="mt-3 text-sm font-semibold leading-tight">{step.nome}</h3>
                  <p className="mt-1 text-[11px] text-muted-foreground">{step.tech}</p>
                </button>
                {idx < PIPELINE_STEPS.length - 1 && <ChevronRight className="h-5 w-5 shrink-0 text-muted-foreground" />}
              </div>
            );
          })}
        </div>
        ...
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1">
            <div className="flex flex-wrap items-center gap-3">
              ...
              <span className={cn("rounded-full border px-2 py-0.5 text-xs", statusStyle[current.status])}>{current.status}</span>
            </div>
      ...
      <div className="grid gap-4 md:grid-cols-3">
        {[
          ...
          { t: "Volume processado", v: "~16.5k", d: "Registros por execução completa" },
